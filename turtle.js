function Turtle() {
	"use strict";
	this.makeColor = function(red, green, blue){return {red:red, green:green, blue:blue};}
	...
	this.pixelIndex = function(x, y){return (y * this.canvas.width) + x;} // Returns the index of an X-Y. For speed -- internal use only.
	this.pixelColor = function(x, y){return this.indexColor(this.pixelIndex(x, y));} // Return's a pixel's color. 0 if out-of-bounds.
	...
	this.colorPixel = function(x, y, rgb){ // Safely colors a pixel -- no chance of out-of-bounds writes.
		if(0<=x && x<this.canvas.width && 0<=y && y<this.canvas.height){
			this.colorIndex(this.pixelIndex(x, y), rgb);
		}
	}
	this.canvasColor = function(){
		let color32 = this.pixelColor(this.x, this.y);
		return {red:(color32 & 0x000000FF), green:(color32 & 0x0000FF00) >> 8, blue:(color32 & 0x00FF0000) >> 16};
	}
	...
	this.moveTo = function(endX, endY){
		...
		let dx = Math.abs(endX-this.x); // x distance
		let sx = Math.sign(endX-this.x); // x sign
		let dy = -Math.abs(endY-this.y);
		let sy = Math.sign(endY-this.y);
		let error = dx+dy;
